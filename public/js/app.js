...
socket.on('whisper', msgObj => {
    ...
    <p class="font-bold text-xs mb-1"></p>
        <div class="messages text-sm text-gray-700 grid grid-flow-row gap-2">
            <div>
                <p>Private message: ${ msgObj.sender }</p>
            </div>
            <div class="flex items-center group">
                <p class="px-6 py-3 rounded-t-full rounded-r-full bg-gray-800 max-w-xs lg:max-w-md text-gray-200">${ msgObj.msg }</p>
            </div>
        </div>
    </div>
    `
...
socket.on('error_whisper', errorMsg => {
